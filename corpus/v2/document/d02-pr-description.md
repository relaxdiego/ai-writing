...
@@ -18,10 +18,12 @@ def read_rows(path):
...
@@ -41,10 +43,16 @@ def import_file(path, account):
-def import_file(path, account):
+def import_file(path, account):
+    errors = []
     created = 0
...
--- a/src/importer/views.py
+++ b/src/importer/views.py
@@ -30,7 +30,11 @@ def upload(request):
-    created = import_file(path, request.account)
-    return render(request, "done.html", {"created": created})
+    result = import_file(path, request.account)
